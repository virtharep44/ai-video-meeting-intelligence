...
**Transform any meeting video or audio into structured intelligence — transcripts, summaries, action items, decisions, speaker diarization, RAG-powered chat, and AI-generated follow-up emails.**
...
- A ready-to-send follow-up email and you can edit it accordingly
...
| Feature | Description |
|---|---|
| 🎙️ **Transcription** | Local Whisper (CPU) for English · Sarvam AI `saaras:v2.5` for Hinglish |
| 📋 **Summarization** | Map-Reduce pattern — chunks → individual summaries → combined final summary |
| ✅ **Action Items** | Extracts tasks, owners, and deadlines from transcript |
| 🔑 **Key Decisions** | Lists all decisions made during the meeting |
| ❓ **Open Questions** | Flags unresolved topics needing follow-up |
| 🎤 **Speaker Diarization** | Energy-based approach — lightweight, no ML model needed |
| 📧 **Email Generator** | AI-drafted professional follow-up email, editable and downloadable |
| 💬 **RAG Chat** | Ask any question about the meeting using ChromaDB + Mistral AI |
| 🌐 **YouTube Support** | Paste any YouTube URL — audio downloads automatically via yt-dlp |
| 📁 **Local File Support** | Upload any `.mp4`, `.mp3`, `.wav`, `.m4a` file |
| 📄 **PDF Export** | Download results as a PDF report via fpdf2 |
| 🌙 **Dark UI** | Custom deep-space Streamlit theme with Syne + JetBrains Mono fonts |
...
| Temperature | 0.2 (extraction) / 0.3 (summary, RAG, email) | Controls output creativity |

### 🎙️ Speech to Text
| Engine | Details | Used For |
|---|---|---|
| **OpenAI Whisper** | Local · CPU · `base` model · `openai-whisper` | English transcription (primary) |
...
| **Chunk Size** | 500 tokens · overlap 50 (for RAG) |
...
| `tiktoken` | 0.7.0 | Token counting (LangChain internals) |
...
1. YouTube URL → yt-dlp → best audio format → FFmpeg → `.wav`
2. Local file → pydub → mono, 16kHz `.wav`
3. WAV → pydub → 10-minute chunks
4. Each chunk → Whisper or Sarvam API (25s sub-slices for Sarvam)
...
> Chose this over `pyannote.audio` and `speechbrain` due to Windows CPU constraints. Lightweight and dependency-free beyond numpy + pydub.
...
- Windows / Linux / macOS (CPU — no GPU needed)
...
> ⚠️ `torch` install can be slow. If you only want CPU torch, use:
> ```bash
...
5. Explore results — Summary, Action Items, Decisions, Questions, Speakers
6. Edit and download the AI-generated follow-up email
7. Chat with your meeting using the RAG chatbot
...
4. LLM Processing (Mistral small-latest via LangChain LCEL)
...
Whisper runs fully locally — no API quota, no latency, no cost. Sarvam integration is preserved in the codebase for Hinglish where its translation capability is valuable.

**Why energy-based diarization?**
`pyannote.audio` and `speechbrain` both require GPU or complex setup on Windows. The custom numpy/pydub energy-based approach works reliably on CPU with zero additional dependencies.

**Why Map-Reduce for summarization?**
Meetings can be long. A single LLM call on a 1-hour transcript would exceed context limits. Map-Reduce chunks the transcript, summarizes each chunk, then combines into a final summary — reliable at any length.
...
yt-dlp==2024.5.27       # YouTube download
...
mistralai==1.0.3        # Mistral SDK
chromadb==0.5.3         # Vector store
...
- **Sarvam routing**: Both English and Hinglish currently route to Whisper. To re-enable Sarvam for Hinglish, update `transcribe_chunk()` in `core/transcriber.py` to call `transcribe_chunk_sarvam()` for `hinglish`.
- **Speaker diarization**: Detects max 2 speakers. Energy-based approach may misclassify in noisy audio.
- **Whisper on CPU**: `base` model is fast but less accurate. Use `small` or `medium` via `WHISPER_MODEL` env var for better results (slower).
- **YouTube cookies**: Some videos require authentication. Place `www.youtube.com_cookies.txt` in the project root.
