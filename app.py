...
with st.sidebar:
    st.markdown(
        '<div class="hero-title" style="font-size:1.6rem">🎬 AI<br>Video Meeting<br>Intelligence</div>',
        unsafe_allow_html=True
    )
    st.markdown("---")
...
st.markdown('<div class="hero-title">AI Video Meeting Intelligence</div>', unsafe_allow_html=True)
...
if run_btn:
    if not source.strip():
        ...
        try:
            ...
            email_content = generate_email(summary, action_items, decisions)
            ...
            update_step("diarize", "active")
            speakers = diarize_audio(chunks[0])
            update_step("diarize", "done")
            ...
            if "RerunData" in str(e) or "RerunException" in str(type(e).__name__):
                raise
            for k in ["audio", "transcript", "title", "summary", "extract", "rag", "diarize"]:
...
if st.session_state.result:
    ...
    if r.get("speakers") is not None:
        speakers_html = '<div class="card"><div class="card-title">🎤 Speaker Diarization</div><div class="card-content">'
        for s in r["speakers"]:
            if "error" not in s:
                speakers_html += f'<div style="margin-bottom:0.5rem">🎙️ <strong>{s["speaker"]}</strong> → {s["start"]}s — {s["end"]}s</div>'
        speakers_html += '</div></div>'
        st.markdown(speakers_html, unsafe_allow_html=True)

    st.markdown("---")
    st.markdown(
        '<div style="font-family:\'Syne\',sans-serif;font-size:1.2rem;font-weight:700;margin-bottom:1rem">📧 AI Email Assistant</div>',
        unsafe_allow_html=True
    )
    ...
    if r.get("email"):
        ...
        with col1:
            if st.button("🔄 Regenerate Email", key="regenerate_email"):
                ...
                st.session_state.result["email"] = new_email
                st.rerun()
        ...
        with col2:
            st.download_button(
                ...
                data=f"To: {recipient}\nSubject: {subject}\n\n{final_email}",
                file_name=f"email_{r['title'][:30].replace(' ', '_')}.txt",
                mime="text/plain",
                key="download_email"
            )
    else:
